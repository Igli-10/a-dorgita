...
class UsuarioDAO
{
    ...
    // Actualiza a ruta da foto de perfil dun usuario
    public function actualizarFotoPerfil($id, $nomeFicheiro)
    {
        try {
            $stmt = $this->conexion->prepare("UPDATE usuarios SET foto_perfil = ? WHERE id = ?");
            return $stmt->execute([$nomeFicheiro, $id]);
        } catch (PDOException $e) {
            error_log("Erro ao actualizar foto: " . $e->getMessage());
            return false;
        }
    }
